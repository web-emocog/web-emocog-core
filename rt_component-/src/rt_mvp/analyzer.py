...
class TrialOutcome:
    ...
    first_press_button: Optional[str]=None
    rt_ms: Optional[float]=None
    press_count: int=0
    premature_press_count: int=0
    late_press_count: int=0
    classification: str="unknown"
    is_correct: bool=False
    is_valid_rt: bool=False
...
def build_trials(log_path: str, task: str, cfg: ProjectConfig) -> Tuple[List[TrialOutcome], Dict[str, Any]]:
    ...
    for tid in sorted(g.keys()):
        ...
        in_window = [(t,b) for (t,b) in press_times if t0 <= t <= t1]
        premature = [(t,b) for (t,b) in press_times if (t0 - prem_ms/1000.0) <= t < t0]
        late = [(t,b) for (t,b) in press_times if t > t1]
        ...
        if task in ("simple","choice"):
            ...
            if is_go is True:
                ...
                if out.rt_ms is None:
                    ...
                    out.classification="commission"; out.is_commission=True
                    if out.rt_ms < min_rt:
...
def compute_metrics(trials: List[TrialOutcome], task: str, cfg: ProjectConfig) -> Dict[str, Any]:
    ...
    return {
        "counts": {"total_trials": total,"correct":correct,"wrong":wrong,"commission":commission,"omission":omission,"anticipation":anticipation,"timeout":timeout,"go_trials":go_trials,"nogo_trials":nogo_trials},
        "rt": {"n_valid":len(rt_valid),"mean_rt_ms":mean_rt,"median_rt_ms":median_rt,"rt_std_ms":rt_std,"rt_cv":rt_cv,"rt_slope_ms_per_trial":rt_slope,"lapses_gt_ms":lapse_ms,"lapses_count":lapses,"lapse_rate":lapse_rate},
        "rates": {"accuracy":accuracy,"omission_rate":omission_rate,"commission_error_rate":commission_rate,"timeout_rate":timeout_rate,"anticipation_rate":anticipation_rate,"hit_rate":hit_rate,"false_alarm_rate":fa_rate,"d_prime":d_prime},
...
def analyze_and_report(log_path: str, task: str, config_path: Optional[str]=None) -> Dict[str, Any]:
    ...
    trials, meta = build_trials(log_path, task, cfg)
    metrics = compute_metrics(trials, task, cfg)
    flags = compute_state_flags(trials, metrics, task, cfg)
    ...
    html = build_report_html(meta, trials, metrics, flags)
