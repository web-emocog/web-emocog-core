...
def _axes(w: int, h: int, pad: int=40) -> Tuple[int,int,int,int,str]:
    ...
    ax=[]
    ax.append(f'<line x1="{x0}" y1="{y1}" x2="{x1}" y2="{y1}" stroke="#000" />')
    ax.append(f'<line x1="{x0}" y1="{y0}" x2="{x0}" y2="{y1}" stroke="#000" />')
    return x0,y0,x1,y1,"\n".join(ax)
...
def svg_scatter(trials: List[TrialOutcome], w: int=900, h: int=320) -> str:
    ...
    for i,t in enumerate(trials, start=1):
        if t.rt_ms is None: continue
        pts.append((i,float(t.rt_ms),t.classification))
    if not pts: return "<p>Нет RT-точек.</p>"
    ...
    for xi,yi,cls in pts:
        cx=_scale(float(xi),float(xmin),float(xmax),x0,x1)
        cy=_scale(float(yi),float(ymin),float(ymax),y1,y0)
        svg.append(f'<circle cx="{cx:.1f}" cy="{cy:.1f}" r="3.3" fill="{color.get(cls,"#000")}" />')
    svg.append(_svg_footer())
    return "\n".join(svg)

def svg_hist(values: List[float], bins: int=12, w: int=900, h: int=260) -> str:
    if not values: return "<p>Нет валидных RT.</p>"
    ...
    for i,c in enumerate(counts):
        bh=0 if maxc==0 else (c/maxc)*(y1-y0)
        x=x0+i*bar_w; y=y1-bh
        svg.append(f'<rect x="{x:.1f}" y="{y:.1f}" width="{bar_w-2:.1f}" height="{bh:.1f}" fill="#78909c" />')
    svg.append(_svg_footer())
    return "\n".join(svg)
...
def svg_trend(trials: List[TrialOutcome], w: int=900, h: int=260) -> str:
    ...
    if len(xs)<2: return "<p>Недостаточно валидных RT.</p>"
    xmin,xmax=min(xs),max(xs); ymin,ymax=min(ys),max(ys)
    ymax=ymax*1.05+1.0; ymin=max(0.0,ymin*0.95-1.0)
    slope=stats.linear_regression_slope(xs,ys) or 0.0
    xm=stats.mean(xs) or 0.0; ym=stats.mean(ys) or 0.0
    a=ym-slope*xm
    x0,y0,x1,y1,ax=_axes(w,h)
    svg=[_svg_header(w,h),ax]
    for xi,yi in zip(xs,ys):
        cx=_scale(xi,xmin,xmax,x0,x1); cy=_scale(yi,ymin,ymax,y1,y0)
        svg.append(f'<circle cx="{cx:.1f}" cy="{cy:.1f}" r="3.2" fill="#2e7d32" />')
    xL,xR=xmin,xmax; yL=a+slope*xL; yR=a+slope*xR
    lx1=_scale(xL,xmin,xmax,x0,x1); lx2=_scale(xR,xmin,xmax,x0,x1)
    ly1=_scale(yL,ymin,ymax,y1,y0); ly2=_scale(yR,ymin,ymax,y1,y0)
    svg.append(f'<line x1="{lx1:.1f}" y1="{ly1:.1f}" x2="{lx2:.1f}" y2="{ly2:.1f}" stroke="#000" stroke-width="2" />')
    svg.append(_svg_footer())
    return "\n".join(svg)

def build_report_html(meta: Dict[str, Any], trials: List[TrialOutcome], metrics: Dict[str, Any], flags: Dict[str, Any]) -> str:
    task=html.escape(str(meta.get("task","")))
    rt_valid=[float(t.rt_ms) for t in trials if t.is_valid_rt and t.rt_ms is not None]
    scatter=svg_scatter(trials); hist=svg_hist(rt_valid); trend=svg_trend(trials)
