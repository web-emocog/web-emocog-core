class EmotionAnalyzer {
    ...
    /**
     * Запуск анализа эмоций
     * Теперь НЕ запускает собственный цикл, а только устанавливает флаг isRunning
     */
    start(videoElement) {
        if (this.isRunning) {
        ...
        console.log('[EmotionAnalyzer] ▶️ Запущен (режим: внешние landmarks)');

        // ❌ УДАЛИЛИ: this.analyzeLoop();
    ...
    processLandmarks(landmarks) {
        ...
        try {
            // Генерируем маску лица
            this.faceMask = this.generateFaceMask(landmarks);

            // Анализ эмоции
            let emotionScores;
            if (this.emotionSession === 'fallback') {
                emotionScores = this.fallbackEmotionAnalysis(landmarks);
            } else {
                emotionScores = this.analyzeEmotionWithModel(landmarks);
            }

            // Сглаживание
            this.smoothingBuffer.push(emotionScores);
            if (this.smoothingBuffer.length > this.config.smoothingWindow) {
                this.smoothingBuffer.shift();
            }

            const smoothedScores = this.smoothEmotionScores();

            // Аффективные измерения
            this.affectiveDimensions = this.calculateAffectiveDimensions(smoothedScores);
            ...
            this.recordEmotionEvent({
                ...
                affective: this.affectiveDimensions,
