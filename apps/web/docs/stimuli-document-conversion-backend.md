# Document-to-stimuli conversion

Status: implemented in `apps/api/routes/stimuli.js` and
`apps/api/stimuli/document-converter.js`.
...
- Store each generated image under the server-owned uploads root and create one
  `stimuli` database row per page in a transaction.
...
- Apply bounded upload/page/image/concurrency/time limits and clean temporary
  source/intermediate/output files.
- Roll back database rows and remove generated files after a partial failure.

## Deployment requirements

- Install headless LibreOffice and Poppler (`pdftoppm`).
- Configure persistent object/file storage. The current process-local
  `uploads/stimuli` directory is insufficient for ephemeral or horizontally
  scaled containers.
- Monitor conversion timeout, rejection and cleanup errors without logging file
