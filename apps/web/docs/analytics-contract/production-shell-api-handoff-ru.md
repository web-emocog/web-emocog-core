...
Статус: frontend API-driven shell и versioned backend endpoints реализованы;
автоматического demo fallback нет.
...
Если endpoint вариантов недоступен, project/protocol/session остаются рабочими, а UI явно сообщает, что block/stimulus/AOI недоступны.
...
`apps/api/analytics/v1-router.js` реализует filter options, immutable snapshots,
session summary/AOI/heatmap, participant-equal group summary/heatmap,
comparison readiness и единый CSV/JSON export по тому же snapshot. Payload
валидируется allowlist-схемами, snapshot привязан к tenant и dataset hash.

Level 2 намеренно не подбирает статистический тест автоматически: до утверждения
метода он возвращает `not_configured`, `insufficient_data` или `confounded`.
