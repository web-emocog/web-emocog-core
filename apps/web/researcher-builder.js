...
function ExperimentBuilderView(options = {}) {
        ...
        <div style="flex:1;display:flex;flex-direction:column;padding:14px;overflow:hidden;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;flex-shrink:0;">
            <div>
              ...
              <div style="font-size:11px;color:var(--muted);line-height:1.45;margin-top:4px;max-width:720px;">
                ${trb('Перед тем, как запустится ваш эксперимент, участник должен будет пройти следующие этапы: информированное согласие (~2 мин), анкета (~1 мин), проверка камеры (~3 мин), калибровка (~3 мин).','Before your experiment starts, the participant will complete these steps: informed consent (~2 min), questionnaire (~1 min), camera check (~3 min), calibration (~3 min).')}
              </div>
            </div>
          </div>
  ...
  function participantProtocolLink() {
    const protocolId = encodeURIComponent(protocolMeta.protocolId || 'protocol');
    return `${location.origin}${location.pathname}#/participant/${protocolId}`;
  }
    ...
    canvasCol.innerHTML = `
      <div style="display:flex;flex-direction:column;height:100%;padding:18px;gap:14px;overflow-y:auto;">
        <div>
          ...
          <div style="font-size:12px;color:var(--muted);margin-top:3px;">${trb('Проверьте собранный эксперимент и подготовьте ссылку для участников.','Review the assembled experiment and prepare the participant link.')}</div>
        </div>
  ...
  function doSave() {
    ...
    const systemBlocks = requiredProtocolBlocks().map((b, i) => ({
    ...
    toast(trb('Протокол Активен и сохранён!','Protocol is active and saved!'));
    setTimeout(() => navigate('#/experiments'), 700);
  }
