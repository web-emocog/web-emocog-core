...
function ExperimentsListView() {
  ...
  function participantLinkForExperiment(exp) {
    const protocolId = exp?.metadata?.protocolId || exp?.protocolId || exp?.id || 'protocol';
    return `${location.origin}${location.pathname}#/participant/${encodeURIComponent(protocolId)}`;
  }
