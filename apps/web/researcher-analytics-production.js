(function (global) {
  ...
  function hasAuth() {
    return !!global.EmocogAnalyticsPreviewFixture
      || !!String(global.API_TOKEN || selectedStored('emocog_api_token') || '').trim();
  }
  ...
  const api = {
    ...
    async exportSnapshot(state, format, content) {
      ...
      const response = await global.fetch(`${base}/analytics/v1/exports?${params.toString()}`, { headers: global.apiHeaders ? global.apiHeaders() : {} });
