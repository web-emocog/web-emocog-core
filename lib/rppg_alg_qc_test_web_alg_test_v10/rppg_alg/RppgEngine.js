...
export function adjustConfidenceForRespiration(
  baseConfidence,
  respOut,
  respCoupling01,
  respirationConfig = {}
) {
  let adjusted = clamp(baseConfidence, 0, 1);
  const respConfidence = respOut?.conf;
  const minConfidence = respirationConfig.minConfForCoupling ?? 0.20;
  const weight = clamp(respirationConfig.couplingWeight ?? 0.25, 0, 1);
  if (
    Number.isFinite(respConfidence)
    && respConfidence >= minConfidence
    && Number.isFinite(respCoupling01)
  ) {
    const factor = (1 - weight) + weight * clamp(respCoupling01, 0, 1);
    adjusted = clamp(adjusted * factor, 0, 1);
  }
  return adjusted;
}
...
export class RppgEngine { // Основной класс для обработки rPPG сигналов
  ...
  update({ timestampMs, frameW, frameH, imageData, landmarks, fps }) { // Обновление состояния
    ...
    const baseConfidence = clamp(confidenceHeuristic * priorFactor, 0, 1);

    const bpmRaw = chosenOut.bpmRaw;
    let bpmSmoothed = this._smoothBpm(bpmRaw, baseConfidence);
    ...
    // Связь ЧСС–дыхание может только понизить базовую уверенность.
    const confidence = adjustConfidenceForRespiration(
      baseConfidence,
      respOut,
      respCoupling01,
      this.cfg.respiration ?? {}
    );
