...
function clampRectFit(r, W, H) {
  if (!r || ![r.x, r.y, r.w, r.h].every((v) => Number.isFinite(v))) {
 ...
 *
 * Файл v5 — новое имя модуля, чтобы обойти долгий кэш ES-модулей по старому URL v4.
 *
 * v4 = v3 +
...
export function roiRectsAndMasksFromLandmarksV4(landmarks, W, H) {
  ...
  if (mouth) {
    ...
    // «chin» ROI удалён (см. комментарий выше про infra); рот пересекается с шеей и щеками.
    for (const k of ["neck", "cheekL", "cheekR"]) {
