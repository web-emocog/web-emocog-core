...
import { roiRectsAndMasksFromLandmarksV4 } from "./roi/roiFromLandmarks_v5.js";
...
function mergeModesDeep(base, patch) {
  if (!patch || !Object.keys(patch).length) return base ? { ...base } : {};
  const out = { ...base };
  for (const k of Object.keys(patch)) {
    const b = base[k] || {};
    const p = patch[k];
    out[k] = { ...b, ...p };
    if (b.tracking || p.tracking) {
      out[k].tracking = { ...(b.tracking || {}), ...(p.tracking || {}) };
    }
  }
  return out;
}
...
export class RppgEngine { // Основной класс для обработки rPPG сигналов
  constructor(opts = {}) {
    const { modes: optsModes, ...optsRest } = opts;
    this.cfg = {
      ...DEFAULTS,
      ...optsRest,
      ...
      modes: mergeModesDeep(DEFAULTS.modes ?? {}, optsModes ?? {}),
    };
  ...
  update({ timestampMs, frameW, frameH, imageData, landmarks, fps }) { // Обновление состояния
    ...
    for (const k of this.roiKeys) {
      ...
      const rk = rects[k];
      if (!rk) continue;
      const st = sampleRectStatsV4(imageData, frameW, frameH, rk, {
    ...
    let confidence = clamp(confidenceHeuristic * priorFactor, 0, 1);
